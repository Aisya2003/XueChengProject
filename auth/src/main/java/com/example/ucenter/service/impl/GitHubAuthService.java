...
public class GitHubAuthService implements IAuthService {
    ...
    public UserExt auth(AuthParamsDto dto) {
        ...
        User user = userMapper.selectOne(
                new LambdaQueryWrapper<User>().eq(!StringUtils.isEmpty(username),
                        User::getUsername,
                        username));
        if (user == null) {
        ...
        if (StringUtils.isEmpty(result)) throw new RuntimeException("用户登录出错！请重新登录");
    ...
    private User getUserInfo(HttpHeaders headers, TokenDto accessToken) {
        ...
        queryWrapper.eq(!StringUtils.isEmpty(githubId), User::getGithubUnionid, githubId);
